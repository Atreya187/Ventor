...
const rawPercentages = scoreEntries.map(([,score]) =>
  totalScore > 0 ? (score / totalScore) * 100 : 0
);
const roundedPercentages = rawPercentages.map(p => Number(p.toFixed(1)));
const roundedTotal = roundedPercentages.reduce((a,b) => a + b, 0);
const residual = Number((100 - roundedTotal).toFixed(1));

if (totalScore > 0 && Math.abs(residual) > 0) {
  const adjustIndex = rawPercentages.reduce((bestIndex, current, index) =>
    current > rawPercentages[bestIndex] ? index : bestIndex
  , 0);
  roundedPercentages[adjustIndex] += residual;
}

const highestMatch = scoreEntries.reduce((best,[model,score]) => {
  if (best[1] === null || score > best[1]) {
    return [model, score];
  }
  return best;
}, [null, null]);

scoreEntries.forEach(([model,score], index)=>{

const percent = roundedPercentages[index].toFixed(1);
...
if (highestMatch[0]) {
  const highestModelName = highestMatch[0].toUpperCase();
  const highestIndex = Object.keys(result.scores).indexOf(highestMatch[0]);
  const highestPercent = totalScore > 0 ? roundedPercentages[highestIndex].toFixed(1) : "0.0";
  document.getElementById("topMatch").innerText = `Top matched model: ${highestModelName} (${highestPercent}%)`;
} else {
...
chartInstance = new Chart(ctx,{ 
  ...
  data:{
    labels:Object.keys(result.scores)
      .map((m,i)=>`${m} (${roundedPercentages[i].toFixed(1)}%)`),
    datasets:[{
